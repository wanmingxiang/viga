    ...
    class MemoryPool
    {
        ...
        public:
            ...
            int init(size_t elem_size, size_t page_size)
            {
                ...
                {
                    fprintf(stderr, "invalid args: elem_size=%lu, page_size=%lu", elem_size, page_size);
                    return -1;
                }
                m_elem_size = adjust_elem_size(elem_size);
                if (m_elem_size > page_size)
                {
                    fprintf(stderr, "page_size is too small: elem_size=%lu, page_size=%lu", elem_size, page_size);
                    return -2;
                }
                m_page_size = page_size;
                fprintf(stderr, "init ok, elem_size=%lu, page_size=%lu", m_elem_size, m_page_size);
                return 0;
            }
            ...
            vaddr_t alloc()
            {
                ...
                if (NULL == page)
                {
                    fprintf(stderr, "failed to alloc new page, page_size=%lu", m_page_size);
                    return NULL;
                }
                try
                {
                ...
                {
                    ::free(page);
                    fprintf(stderr, "failed to push back new page, pages_size=%u", (uint32_t)m_pages.size());
                    return NULL;
                }
...
#ifndef VIGA_PATE_SIZE
#define VIGA_PAGE_SIZE  (1024*1024) /* 1M */
#endif
...
class MultiMemoryPool
{
    ...
    public:
        ...
        int register_item(uint32_t elem_size)
        {
            if (elem_size <= 0)
            {
                fprintf(stderr, "invalid args: elem_size=%u", elem_size);
                return -1;
            }
            m_pools[elem_size] = NULL;
            fprintf(stderr, "register item: elem_size=%u", elem_size);
            return 0;
        }
        ...
        int init(uint32_t /*max_items_num*/)
        {
            ...
            while (it != m_pools_end)
            {
                ...
                if (NULL == it->second)
                {
                    fprintf(stderr,"failed to alloc MemoryPool");
                    return -1;
                }
                if (0 > it->second->init(it->first, VIGA_PAGE_SIZE))
                {
                    fprintf(stderr, "failed to init MemoryPool, elem_size=%u, page_size=%u", it->first, VIGA_PAGE_SIZE);
                    return -1;
                }
                ++it;
            }
            fprintf(stderr, "init ok, elems num=%u, page_size=%u", (uint32_t)m_pools.size(), VIGA_PAGE_SIZE);
            return 0;
        }
        ...
        vaddr_t alloc(uint32_t elem_size)
        {
            ...
            if (it == m_pools_end)
            {
                fprintf(stderr, "unregistered elem size: %u", elem_size);
                return NULL;
            }
            if (NULL == it->second)
            {
                fprintf(stderr, "not init elem size: %u", elem_size);
                return NULL;
            }
        ...
        void print_meta() const
        {
            ...
            while (it != m_pools.end())
            {
                if (it->second)
                {
                    ...
                    total_used += mem_used;
                    fprintf(stderr, "pool[%d]: elem_size=%u, mem used=%llu", i, it->first, (uint64_t)mem_used);
                    ++i;
                }
                ++it;
            }
            fprintf(stderr, "total mem used=%llu", (uint64_t)total_used);
        }
    private:
        Map m_pools;
