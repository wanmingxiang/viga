...
class TObjectPool
{
    ...
    public:
        ...
        int init(Pool *pool)
        {
            if (NULL != m_pool)
            {
                fprintf(stderr,"ignore duplicate init call");
                return -1;
            }
            m_pool = pool;
            if (NULL == m_pool)
            {
                ...
                if (NULL == m_pool)
                {
                    fprintf(stderr,"failed to new Pool");
                    return -1;
                }
                if (init_pool(m_pool) < 0)
                {
                    fprintf(stderr,"failed to register cleanup_bat_t");
                }
                else if (m_pool->register_item(sizeof(T)) < 0)
                {
                    fprintf(stderr,"failed to register item");
                }
                ...
                {
                    fprintf(stderr,"use private pool");
                    return 0;
                }
                delete m_pool;
                m_pool = NULL;
        ...
        int delay_free(vaddr_t ptr, cleanup_fun_t fun = NULL, intptr_t arg = 0)
        {
            ...
            if (fun)
            {
                ...
                if (NULL == bag)
                {
                    fprintf(stderr,"failed to alloc cleanup_bag_t");
                    return -1;
                }
                bag->fun = fun;
                bag->arg = arg;
