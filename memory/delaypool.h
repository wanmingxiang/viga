...
class TDelayPool
{
    ...
    public:
        ...
        int init(uint32_t max_items_num)
        {
            ...
            if (ret < 0)
            {
                fprintf(stderr,"failed to register for node_t");
                return -1;
            }
            ret = m_pool.init(max_items_num);
            if (ret < 0)
            {
                fprintf(stderr,"failed to init m_pool");
                return -1;
            }
        ...
        int delay_free(vaddr_t ptr, uint32_t elem_size, destroy_fun_t fun = NULL, intptr_t arg = 0)
        {
            ...
            if (NULL == node)
            {
                fprintf(stderr,"failed to alloc list node");
                return -2;
            }
            node->next = 0;
            node->ptr = ptr;
        ...
        {
            m_pool.print_meta();
            fprintf(stderr,"delayed elem num=%lu", (uint64_t)m_delayed_num);
        }
    private:
        TMemoryPool m_pool;
